 ...
 * 
 * Found in http://www.gdreflections.com/2011/02/hexagonal-grid-math.html
 * 
 * Modified for Android by luisollero@gmail.com
 * 
 */
public class HexGridCell {

	private static final String TAG = HexGridCell.class.getSimpleName();
    ...
    /**
     * Creates the {@link Path} for painting the hexagon
     * 
    ...
    public Path createPath() {
    	Path path = new Path();
    	// Initial corner
        path.moveTo(mX + CORNERS_DX[0], mY + CORNERS_DY[0]);
        for (int i = 0; i < 6; i++) {
        	// Drawing to the next corner
        	path.lineTo(mX + CORNERS_DX[i], mY + CORNERS_DY[i]); 
		}
        // Final corner
        path.lineTo(mX + CORNERS_DX[0], mY + CORNERS_DY[0]); 
        return path;
    }
